...
eavesdropper=1:49;
smooth_totalosj = smooth(totalosj,15);
smooth_totalos = smooth(totalos,15);
smooth_totaloscj = smooth(totaloscj,15);

% Plot the smoothed lines
figure;
x=plot(eavesdropper,smooth_totalosj,'-x');
x.LineWidth=1; 
hold on;
y=plot(eavesdropper,smooth_totalos,'-o');
y.LineWidth=1; 
z=plot(eavesdropper,smooth_totaloscj,'--');
z.LineWidth=1; 
z.Color = [0 0 0];  
grid on;
